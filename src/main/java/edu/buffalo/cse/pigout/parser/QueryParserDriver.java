...
import edu.buffalo.cse.pigout.parser.QueryParserUtils;
...
// QueryParserDriver for PigOut script. As PigOut uses almost same script 
// language as Pig Latin, most codes were just copied from Pig's. 
// However, PigOut has to generate LogicalPlan in a different way,
// so a new class is created.
// Kyungho Jeon, 2013.09.05
...
public class QueryParserDriver {
    ...
    public QueryParserDriver(PigContext context, String scope, Map<String, String> fileNameMap) {
...
*/
    public Tree getParseTree(String query) throws ParserException {
    ...
    public LogicalPlan parse(String query) throws ParserException {
        ...
        try {
            ...
            LogicalPlanGenerator planGenerator =
                new LogicalPlanGenerator( new CommonTreeNodeStream( ast ), 
                		this.context, scope, fileNameMap );
            planGenerator.query(); //
    ...
    Tree expandMacro(Tree ast) throws ParserException {
        ...
        Map<String, PigOutMacro> seen = new HashMap<String, PigOutMacro>();
        List<PigOutMacro> macroDefs = new ArrayList<PigOutMacro>();
    ...
    */
    ...
    private void inlineMacro(List<CommonTree> inlineNodes,
            List<PigOutMacro> macroDefs) throws ParserException {
        for (CommonTree t : inlineNodes) {
            Set<String> macroStack = new HashSet<String>();
            CommonTree newTree = PigOutMacro.macroInline(t, macroDefs, macroStack, pigContext);
    ...
    private PigServer getPigServer() throws ExecException {
        if (pigServer == null) {
            pigServer = new PigServer(this.context, false);
        }
        return pigServer;
    }
            ...
            if (fnameMap.get(fname) != null) {
              ...
              try {
                ...
                localFileRet = localFile == null ?
                        FileLocalizer.fetchFile(this.context.getProperties(), fname)
     ...
     */
    private PigOutMacro makeMacroDef(CommonTree t, Map<String, PigOutMacro> seen)
            throws ParserException {
        ...
        PigOutMacro pm = new PigOutMacro(mn, fname, params, returns, body, seen);
    ...
    private void macroImport(CommonTree t) throws ParserException {
        ...
        Tree macroAST = null;
        if (context.macros.containsKey(fname)) {
            macroAST = context.macros.get(fname);
        } else {
            FetchFileRet localFileRet = getMacroFile(fname);
            ...
            try {
                ...
                macroText = context.doParamSubstitution(in);
            } catch (IOException e) {
                String msg = getErrorMessage(fname, t,
            ...
            try {
                ...
                context.macros.put(fname, macroAST);
            } catch(RuntimeException ex) {
                throw new ParserException( ex.getMessage() );
            }
        }

...
import edu.buffalo.cse.pigout.tools.pigoutcmd.PigOutSh;
...
/**
 * PigOut Main class based on Pig's Main 
 */

/**
 * Let's make this simple. 
 * Do not take scripts in command line. Instead, only allow -f option
 */
...
public class Main {
    ...
    static {
    	...
    	try {
    		...
    		if (findContainingJar != null) {
    			...
    			attr = attrs.get("edu/buffalo/cse/pigout");
    			jar.close();
    		} else {
    			log.info("Unable to read PigOut manifest file as we are not running from a jar, version information unavailable");
    		}
    	} catch (Exception e) {
    		log.warn("Unable to read PigOut manifest file, version information unavailable", e);
    	}
    ...
    static int run(String args[], PigProgressNotificationListener listener) {
        ...
        try {
            ...
            PigContext pigContext = 
            		new PigContext( execType, properties );
            ...
            case FILE: {
            	...
            	if (remainders != null) {
            		pigContext.getProperties().setProperty(
            				PigContext.PIG_CMD_ARGS_REMAINDERS,
  ...
  //TODO jz: log4j.properties should be used instead
    ...
    {
        // The following usage help must be revised... 
        // 8.23.2013
        System.out.println("\n");
            ...
            System.out.println("    -x, -exectype - Set execution mode: local|single|multi, default is multi.");
...
//        return "PigOut version " + version + " (r" + svnRevision + ") \ncompiled "+buildTime;
        return "PigOut version 0.1";
    }
